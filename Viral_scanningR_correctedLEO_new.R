...
## ---------------------------
##
## Script name: VIRAL TRACK: PART ONE - Viral_Scanning 
## Function: Map Viral Reads using STAR and generate QC metrices 
...
log_con <- file("Viral_track_scanning.log")

## ---------------------------------------------------------------
## Commensing Script
## ---------------------------------------------------------------
## Loading of the libraries
cat("Loading of libraries....  \n", file="Viral_track_scanning.log", append = TRUE)
suppressMessages(library(Biostrings))
suppressMessages(library(ShortRead))
suppressMessages(library(doParallel))
suppressMessages(library(GenomicAlignments))
suppressMessages(library(ggplot2))                  #LEO ADDITION
cat("Libraries Loaded \n", file="Viral_track_scanning.log", append = TRUE)



## ---------------------------------------------------------------
## First step : Parameter loading and checking
## Loading and pre-processing

cat("Parameter Loading from Paramater File \n", file="Viral_track_scanning.log", append = TRUE)
...
cat('Paramaters Loaded and Checked \n', file="Viral_track_scanning.log", append = TRUE)
...
## ---------------------------------------------------------------
## Second step : Target Fastq file identification

cat("Identification of Target Fastqs from input.file.txt \n", file="Viral_track_scanning.log", append = TRUE)
Parameter_target_files = args[2]
#Parameter_target_files = '/well/immune-rep/users/kvi236/VIRUS/VIRAL_TRACK/Files_to_process_downsampled.txt'
...
cat("Checking Validitity of Input FASTQs \n", file="Viral_track_scanning.log", append = TRUE)
## Checking the values
List_target_path = c()
...
### Check input and other criteria:
if (length(List_target_path)==0) {
  stop(cat("No Fastq files identified : check the parameters! \n", file="Viral_track_scanning.log", append = TRUE))
}
if (length(List_target_path)>0) {
  cat(paste(length(List_target_path),"Fastq files are going to be processed! \n"), file="Viral_track_scanning.log", append = TRUE)
}

cat('Input files checked and validated \n', file="Viral_track_scanning.log", append = TRUE)
## ---------------------------------------------------------------

cat("Loading required modules to environment - if required \n", file="Viral_track_scanning.log", append = TRUE)
...
cat("Third Step: Commencing Mapping \n", file="Viral_track_scanning.log", append = TRUE)
#For each Fastq file a sub-directory is created
#It will contains the SAM file as well as the final log file 
cat("For each Fastq file a new sub-directory will be created containing output files \n", file="Viral_track_scanning.log", append = TRUE)
List_output_path = c()
...
for (k in List_target_path) {
  ...
  STAR_mapping_command = paste("STAR --runThreadN",N_thread,"--outBAMsortingThreadN 3 --outBAMsortingBinsN 60 --genomeDir",Index_genome,"--readFilesIn",k,"--outSAMattributes NH HI AS nM NM XS ",
  ...
  cat(paste0("Mapping of",name_target,".fastq done ! \n"), file="Viral_track_scanning.log", append = TRUE)
...
### ------------------------------------------------------------------------------------
### ------------------------------------------------------------------------------------
##  Fourth step : QC and analysis of the mapping itself:
...
for (k in List_output_path) {
  ...
  cat("Calculated temp_chromosome_count", file="Viral_track_scanning.log", append = TRUE)
  ...
  dir.create(paste(k,"HUMAN_BAM_files",sep = ""))
  ...
  foreach(i=rownames(temp_chromosome_count_2)) %dopar% {
    ...
    temp_export_bam_command = paste("samtools view -b",temp_sorted_bam,i,">",paste(k,"HUMAN_BAM_files/",i,".bam ",sep = ""))
    temp_export_bam_command = paste("samtools view -b ",temp_sorted_bam," \'",i,"\'"," > \'",k,"HUMAN_BAM_files/",i,".bam\'",sep = "")
  ...
  cat(paste0("Exporting QC ",name_target,".... \n"), file="Viral_track_scanning.log", append = TRUE)
  ...
  if (length(detected_virus)==0) {
    ...
    if (length(detected_virus)>0) {
    cat(paste0(length(detected_virus)," viral sequences detected detected \n"), file="Viral_track_scanning.log", append = TRUE)
  }
  cat(paste0("Exporting QC table for ",name_target,".... \n"), file="Viral_track_scanning.log", append = TRUE)
  ...
  ### Edited by LEO as didn't recognise '\' and compiled choromosome was named without 'chr'
  ...
  Splice_table$Annotated[Splice_table$Annotated==1]="GT_AG"
  Splice_table$Annotated[Splice_table$Annotated==2]="CT_AC"
  Splice_table$Annotated[Splice_table$Annotated==3]="GC_AG"
  Splice_table$Annotated[Splice_table$Annotated==4]="CT_GC"
  Splice_table$Annotated[Splice_table$Annotated==5]="AT_AC"
  Splice_table$Annotated[Splice_table$Annotated==6]="GT_AT"
  Splice_table$Annotated[Splice_table$Annotated==0]="Non-canonical"
  Splice_table$Annotated = factor(Splice_table$Annotated, levels=c("GT_AG","CT_AC","GC_AG","CT_GC","AT_AC","GT_AT","Non-canonical"))
  Splice_table = Splice_table[as.character(Splice_table$Virus)%in%detected_virus,]
  ...
  barplot(Mapping_information$Length_vector,col="black",names.arg = c("Mapping length","Insertion length","Deletion length"), main = "Size of Mapped Reads", 
  ...
  barplot(Mapping_information$Rate_vector,col="black",names.arg = c("Mismatch rate","Insertion rate","Deletion rate"), main = "Rate of Mismatching",
  ...
  plot(QC_result$N_unique_reads,QC_result$Spatial_distribution*100,pch=21,bg=Color_vector,log="x",cex=1.5,xlab="N unique reads mapped",ylab="% Mapped genome",ylim=c(0,100),cex.lab=1.5,main="Mapping Summary per detected Virus: Read Count")
  abline(h=5,lwd=2,lty=2,col="grey")
  abline(v=Minimal_read_mapped,lwd=2,lty=2,col="grey")
  
  #Second QC for the viral hits 
  plot(QC_result$Sequence_entropy,QC_result$Spatial_distribution*100,pch=21,bg=Color_vector, cex=1.5,xlab="Sequence complexity",ylab="% Mapped genome",cex.lab=1.5,ylim=c(0,100),main="Mapping Summary per detected Virus: Sequence Complexity")
  abline(h=5,lwd=2,lty=2,col="grey")
  abline(v=1.2,lwd=2,lty=2,col="grey")
  
  #Third QC for the viral hits 
  plot(QC_result$Longest_contig,QC_result$DUST_score,pch=21,bg=Color_vector, cex=1.5,xlab="Longest contig (nt)",ylab="DUST score",cex.lab=1.4,main="Mapping Summary per detected Virus: DUST Score")
